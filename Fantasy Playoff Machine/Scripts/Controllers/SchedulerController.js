var app = angular.module("fantasyPlayoffMachine").controller("SchedulerController",
	["$scope", "PreloadService",
		function ($scope, preloadService) {
			...
			$scope.calculateAgainstAllPlayersAllWeeks = function () {
			...
			$scope.calculateAgainstAllSchedules = function () {
				_.each($scope.league.CompletedSchedule, function (week) {

				})

				//Go through all completed weeks
					//Go through all matchups
						//Add matchup to each team as a schedule

				//Go through each teams schedule
					//


			}

			$scope.calculateAgainstAllSchedules();
			$scope.calculateAgainstAllPlayersAllWeeks();
		}
	]
);