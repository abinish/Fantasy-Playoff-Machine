var app = angular.module("fantasyPlayoffMachine").controller("Test",
	["$scope", "PreloadService", "$window", "$sce", "OverlayService","$http", "$location", "$timeout",
		function ($scope, preloadService, $window, $sce, overlayService, $http, $location, $timeout) {
			$scope.leagueIDToAdd;
			$scope.leagueName;
			...
			$scope.addLeague = function () {
				...
				$http.get("/Home/TestLeague?leagueId=" + $scope.leagueIDToAdd + "&s2=" + $scope.s2 + "&swid=" + $scope.swid)
					.then(function (data) {
